...
from HeavenMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from HeavenMusic.utils.decorators import ActualAdminCB, AdminActual, language
from HeavenMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Heaven.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Heaven.stop_stream_force(chat_id)
        except:
            pass
